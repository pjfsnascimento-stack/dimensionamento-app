# ==============================================================================
# PLATAFORMA DE WORKFORCE MANAGEMENT (WFM) - VERSÃO 5.0 (PREVISÃO AVANÇADA)
# Corrige o bug de parsing de data e adiciona um módulo de simulação completo.
# ==============================================================================
...
def process_uploaded_file(uploaded_file, column_mapping):
    ...
    try:
        ...
        encoding, delimiter = 'utf-8', ';' # Fallback para o delimitador conhecido
    ...
    # CORREÇÃO CRÍTICA: Força o parsing do formato de data específico AAAA:MM:DD
...
if st.session_state.processed_data is not None:
    ...
    with tab2:
        # ... (código da aba Análise Detalhada) ...
    ...
    # --- ABA 5: PREVISÃO E SIMULAÇÃO ---
    with tab5:
        st.header("Previsão e Simulação de Cenários")
        
        # Seção 1: Simulador Rápido
        ...
        # Seção 2: Simulador Baseado em Dados
        with st.expander("📈 Simulador Baseado em Dados Históricos"):
            ...
            dias_ordem_pt_sim = ['Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado', 'Domingo']
            sim_col_data1, sim_col_data2 = st.columns(2)
            sel_dia = sim_col_data1.selectbox("Selecione o Dia da Semana", dias_ordem_pt_sim, key="sim_dia")
            sel_hora = sim_col_data2.selectbox("Selecione a Hora", range(24), key="sim_hora")
            
            # Busca os dados reais para o horário selecionado
            dados_reais = demand_df[(demand_df['Dia da Semana'] == sel_dia) & (demand_df['Hora'] == sel_hora)]
            
            if not dados_reais.empty:
                vol_real = dados_reais['chamadas_por_hora'].iloc[0]
                tma_real = dados_reais['tma_medio'].iloc[0]
