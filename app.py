# ==============================================================================
# PLATAFORMA DE WORKFORCE MANAGEMENT (WFM) - VERSÃO 15.0 (EDIÇÃO FINANCEIRA)
# Reintroduz a Análise de Custos, corrige bugs críticos na IA e no cálculo de escala,
# e eleva a qualidade dos dashboards para o padrão de Business Intelligence.
# ==============================================================================
...
from prophet.plot import plot_components_plotly # CORREÇÃO: Importa a função correta
...
def calculate_required_staff(_demand_df, meta_sl, meta_tempo):
    # O cache foi removido para garantir que recalcule com novos parâmetros de meta
...
if st.session_state.processed_data is not None:
    ...
    with tab2:
        ...
        # Recalcula a escala com base nos parâmetros atuais
    ...
    with tab3:
        ...
        if st.button("🚀 Gerar Previsão com IA", use_container_width=True, type="primary"):
            with st.spinner("A IA está aprendendo e gerando a previsão..."):
                ...
                st.subheader("Análise dos Componentes da Previsão"); fig_components = plot_components_plotly(model, forecast); st.plotly_chart(fig_components, use_container_width=True)
    ...
    with tab4:
        ...
        if total_payroll > 0 and 'Condomínio' in df.columns and 'Atendente' in df.columns:
            ...
            cost_per_hour = total_payroll / (total_agents * horas_produtivas_mes)
            cost_per_second = cost_per_hour / 3600
            df_filtered['custo_atendimento'] = df_filtered['duracao_atendimento'] * cost_per_second
            
            cost_by_condo = df_filtered.groupby('Condomínio').agg(Custo_Total_Rs=('custo_atendimento', 'sum')).sort_values('Custo_Total_Rs', ascending=False).reset_index()
            cost_by_condo['Percentual_Custo'] = (cost_by_condo['Custo_Total_Rs'] / cost_by_condo['Custo_Total_Rs'].sum()) * 100
            cost_by_agent = df_filtered.groupby('Atendente').agg(Valor_Atendido_Rs=('custo_atendimento', 'sum')).sort_values('Valor_Atendido_Rs', ascending=False).reset_index()
            ...
            fig_pareto = go.Figure(); fig_pareto.add_trace(go.Bar(x=cost_by_condo['Condomínio'], y=cost_by_condo['Percentual_Custo'], name='Custo (%)')); fig_pareto.add_trace(go.Scatter(x=cost_by_condo['Condomínio'], y=cost_by_condo['Cum_Percentual'], name='Acumulado (%)', yaxis='y2')); st.plotly_chart(fig_pareto.update_layout(title_text="<b>Concentração de Custo por Cliente</b>", yaxis2=dict(overlaying='y', side='right')), use_container_width=True)
