# ==============================================================================
# PLATAFORMA DE WORKFORCE MANAGEMENT (WFM) - VERSÃO 8.0 (COM MACHINE LEARNING)
# Integra um modelo de previsão de séries temporais (Prophet) para gerar escalas futuras.
# ==============================================================================
...
# ==============================================================================
# ... (As funções de processamento e cálculo da v7.0 permanecem as mesmas) ...
@st.cache_data
...
def prepare_data_for_prophet(_df):
    """Prepara os dados para o modelo Prophet, agregando chamadas por hora."""
...
def generate_forecast(model, start_date, end_date):
    """Gera a previsão para o período futuro especificado."""
    future_periods = (end_date - start_date).days * 24
    future = model.make_future_dataframe(periods=future_periods, freq='H')
    forecast = model.predict(future)
    
    # Filtra para manter apenas o período selecionado pelo usuário
...
def generate_ai_staffing_schedule(_forecast, _historical_demand, meta_sl, meta_tempo, shrinkage, cost_per_hour):
    """Aplica o Erlang C sobre a previsão da IA para gerar a escala."""
    ...
    forecast_schedule['Dia da Semana'] = forecast_schedule['ds'].dt.day_name(locale='pt_BR.utf8')
    forecast_schedule['Hora'] = forecast_schedule['ds'].dt.hour

    # Usa o TMA médio histórico como uma premissa para o futuro
    ...
    final_schedule['tma_medio'].fillna(_historical_demand['tma_medio'].mean(), inplace=True) # Fallback para TMA médio geral
...
with st.sidebar:
    ...
    if uploaded_history:
        ...
        column_mapping['data_hora_inicio'] = st.selectbox("Data/Hora da Chamada", header_df.columns, key="map_time")
        column_mapping['duracao_atendimento'] = st.selectbox("Duração (em s)", header_df.columns, key="map_dur")
    ...
    growth_forecast = st.slider("Previsão de Crescimento (%)", -20, 100, 0)
...
if st.session_state.demand_df is not None:
    ...
    tab_titles = [" dashboards Visão Geral", "💡 Otimização de Escala", "🧠 Previsão com IA"]
            ...
            if start_date > end_date:
                ...
                with st.spinner("A IA está aprendendo com seus dados e gerando a previsão... Isso pode levar alguns minutos."):
                    try:
                        ...
                        # Gera a escala baseada na previsão
                        ...
                        fig_forecast.update_layout(title_text="Previsão de Chamadas por Hora (Linha Preta = Histórico, Azul = Previsão da IA)", xaxis_title="Data", yaxis_title="Volume de Chamadas")
                        st.plotly_chart(fig_forecast, use_container_width=True)
                        
                        st.subheader("Escala Recomendada pela IA para o Período")
                        st.dataframe(final_schedule[['ds', 'Dia da Semana', 'Hora', 'chamadas_previstas', 'tma_medio', 'Escala Prevista com IA', 'Custo Previsto (R$)']].rename(columns={'ds': 'Data'}), hide_index=True)
