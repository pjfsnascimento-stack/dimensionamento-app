# ==============================================================================
# PLATAFORMA DE WORKFORCE MANAGEMENT (WFM) - VERSÃO 12.1 (ESTÁVEL)
# Corrige bugs críticos no Modo Demo e na Previsão com IA.
# ==============================================================================
...
import numpy as np # <-- CORREÇÃO 1: Importa a biblioteca numpy diretamente.
...
def calculate_required_staff(_demand_df, meta_sl, meta_tempo):
    ...
    def _get_required_staff_binary_search(chamadas, tma, meta_sl, meta_tempo):
        # CORREÇÃO 2: Adiciona uma "guarda" para prevenir o erro do ErlangC com transações <= 0.
...
def generate_ai_staffing_schedule(_forecast, _historical_demand, meta_sl, meta_tempo, shrinkage):
    ...
    # A chamada agora passa para a função corrigida, que lida com chamadas <= 0
...
with st.container(border=True):
    ...
    if demo_mode:
        # CORREÇÃO 1: Usa np.random.randint em vez do atalho depreciado pd.np.random.randint
    ...
    with tab3:
        ...
        if st.button("🚀 Gerar Previsão com IA", use_container_width=True, type="primary"):
            with st.spinner("A IA está aprendendo e gerando a previsão..."):
                ...
                st.subheader("Gráfico Interativo da Previsão"); fig_forecast = plot_plotly(model, forecast); st.plotly_chart(fig_forecast, use_container_width=True)
