# ==============================================================================
# PLATAFORMA DE WORKFORCE MANAGEMENT (WFM) - VERSÃO 14.0 (EDIÇÃO FINANCEIRA)
# Adiciona um módulo completo de análise de custos, BI e insights de IA financeira.
# ==============================================================================
...
st.set_page_config(
    page_title="WFM Pro AI - Plataforma de Business Intelligence",
    page_icon="💲",
    layout="wide"
)
...
def generate_cost_ai_report(_cost_by_condo, _cost_by_agent, _cost_per_call):
    """Gera um relatório textual com insights da análise financeira."""
    if _cost_by_condo.empty or _cost_by_agent.empty:
        return "Dados insuficientes para gerar o relatório financeiro."
    ...
    report = f"""
    ### Relatório Executivo da Análise de Custos
    
    Com base nos dados financeiros e operacionais fornecidos, a IA identificou os seguintes pontos-chave:

    **1. Custo por Atendimento:**
    - O custo médio para cada atendimento realizado no período foi de **R$ {_cost_per_call:,.2f}**. Este é o seu principal indicador de eficiência.

    **2. Análise de Custo por Cliente (Condomínio):**
    - O cliente que mais consumiu recursos da operação foi o **{top_condo['Condomínio']}**, com um custo total de **R$ {top_condo['Custo Total (R$)']:,.2f}**, representando **{top_condo['Percentual do Custo (%)']:.1f}%** de todo o custo alocado. É fundamental analisar a rentabilidade e os processos deste cliente.

    **3. Análise de Performance por Atendente:**
    - O operador que mais agregou valor em atendimentos foi **{top_agent['Atendente']}**, responsável por um total de **R$ {top_agent['Valor Atendido (R$)']:,.2f}** em custos de atendimento.

    **Recomendação Estratégica:** Utilize o dashboard abaixo para explorar a relação Custo vs. Volume de cada cliente. Clientes com alto custo e baixo volume podem indicar problemas de processo ou necessidade de renegociação.
    """
    return report
...
st.title("💲 WFM Pro AI: Plataforma de Business Intelligence Financeiro")
...
if st.session_state.processed_data is not None:
    ...
    with tab1:
        # ... (código do dashboard personalizável) ...
        ...
        if "Volume vs. TMA por Hora" in dashboard_components: st.markdown("---"); st.write("#### Análise Interativa: Volume vs. TMA por Hora"); hourly_agg = demand_df_filtered.groupby('Hora', as_index=False).agg({'chamadas_por_hora': 'mean', 'tma_medio': 'mean'}); fig = go.Figure(); fig.add_trace(go.Bar(x=hourly_agg['Hora'], y=hourly_agg['chamadas_por_hora'], name='Volume de Chamadas', yaxis='y1', text=hourly_agg['chamadas_por_hora'].round(1))); fig.add_trace(go.Scatter(x=hourly_agg['Hora'], y=hourly_agg['tma_medio'], name='TMA Médio (s)', yaxis='y2', mode='lines+markers')); fig.update_layout(template="plotly_white", title_text="<b>Volume vs. TMA por Hora</b>", yaxis=dict(title='Volume Médio'), yaxis2=dict(title='TMA (s)', overlaying='y', side='right'), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)); st.plotly_chart(fig, use_container_width=True)
        if "Performance por Atendente" in dashboard_components and 'Atendente' in df.columns: st.markdown("---"); st.write("#### Performance por Atendente"); agent_perf = df_filtered.groupby('Atendente').agg(chamadas_atendidas=('data_hora_inicio', 'count'), tma_medio=('duracao_atendimento', 'mean')).sort_values(by='chamadas_atendidas', ascending=False).reset_index(); fig_agent = px.bar(agent_perf.head(20), x='Atendente', y='chamadas_atendidas', color='tma_medio', text_auto='.2s', title="Top 20 Atendentes por Volume vs. TMA Médio", color_continuous_scale='RdYlGn_r'); fig_agent.update_layout(title_text="<b>Top 20 Atendentes por Volume vs. TMA Médio</b>", template="plotly_white"); st.plotly_chart(fig_agent, use_container_width=True)
        if "Demanda por Fila/Cliente" in dashboard_components and 'Condomínio' in df.columns: st.markdown("---"); st.write("#### Demanda por Fila/Cliente"); condo_perf = df_filtered.groupby('Condomínio').agg(chamadas_atendidas=('data_hora_inicio', 'count'), tma_medio=('duracao_atendimento', 'mean')).sort_values(by='chamadas_atendidas', ascending=False).reset_index(); fig_condo = px.treemap(condo_perf, path=[px.Constant("Todos"), 'Condomínio'], values='chamadas_atendidas', color='tma_medio', title='Distribuição de Volume e TMA por Cliente', color_continuous_scale='Blues', custom_data=['tma_medio']); fig_condo.update_traces(textinfo="label+value", hovertemplate='<b>%{label}</b><br>Chamadas: %{value}<br>TMA Médio: %{customdata[0]:.0f}s'); fig_condo.update_layout(title_text="<b>Distribuição de Volume e TMA por Cliente</b>", template="plotly_white"); st.plotly_chart(fig_condo, use_container_width=True)
    ...
    with tab3:
        ...
        if st.button("🚀 Gerar Previsão com IA", use_container_width=True, type="primary"):
            with st.spinner("A IA está aprendendo e gerando a previsão..."):
                ...
                st.subheader("Gráfico Interativo da Previsão"); fig_forecast = plot_components_plotly(model, forecast); st.plotly_chart(fig_forecast, use_container_width=True)
    ...
    with tab4:
        ...
        if total_payroll > 0 and total_agents > 0 and 'Condomínio' in df.columns and 'Atendente' in df.columns:
            horas_produtivas_mes = st.number_input("Horas Produtivas por Colaborador/Mês", value=176, help="Jornada de trabalho mensal padrão, descontando feriados e folgas. Comumente entre 160 e 180.")
            ...
            cost_by_condo = df_filtered.groupby('Condomínio').agg(
                Custo_Total_Rs=('custo_atendimento', 'sum'),
                Total_Atendimentos=('data_hora_inicio', 'count')
            ).sort_values('Custo_Total_Rs', ascending=False).reset_index()
            cost_by_condo['Percentual_Custo'] = (cost_by_condo['Custo_Total_Rs'] / cost_by_condo['Custo_Total_Rs'].sum()) * 100
            
            cost_by_agent = df_filtered.groupby('Atendente').agg(
                Valor_Atendido_Rs=('custo_atendimento', 'sum'),
                Total_Atendimentos=('data_hora_inicio', 'count'),
                TMA_Medio_s=('duracao_atendimento', 'mean')
            ).sort_values('Valor_Atendido_Rs', ascending=False).reset_index()
            ...
            fig_pareto.update_layout(yaxis=dict(title='Custo Individual (%)'), yaxis2=dict(title='Custo Acumulado (%)', overlaying='y', side='right'))
            st.plotly_chart(fig_pareto, use_container_width=True)
            st.caption("Identifique os 20% de clientes que representam 80% dos seus custos operacionais.")
